...
import { ArrowLeft, MapPin, ShoppingCart, Plus, Minus, Trash2, Ticket, ChevronUp } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-primary text-primary-foreground safe-area-inset-top">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          {items.length > 0 && (
            <button onClick={clearCart} className="touch-manipulation">
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-3">
            {items.map((item) => (
              <div
                key={item.id}
                className="flex gap-3 bg-card rounded-2xl p-3 shadow-soft animate-fade-in"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button
                      className="text-muted-foreground hover:text-destructive touch-manipulation p-1 rounded-full hover:bg-destructive/10 transition-colors"
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <div className="flex items-center gap-1.5 bg-primary rounded-full px-1 py-0.5">
                      <button
                        className="w-6 h-6 rounded-full flex items-center justify-center text-primary-foreground hover:bg-primary-foreground/10 touch-manipulation transition-colors"
                        onClick={() => updateQuantity(item.id, item.quantity - 1)}
                      >
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="font-bold text-xs w-4 text-center text-primary-foreground">
                        {item.quantity}
                      </span>
                      <button
                        className="w-6 h-6 rounded-full flex items-center justify-center text-primary-foreground hover:bg-primary-foreground/10 touch-manipulation transition-colors"
